...
<template>
    <div class="flex flex-row items-center w-full space-x-5 hover:bg-gray-100 py-5">
        ...
        <div class="flex flex-col text-sm space-y-5 w-full text-xs ">
            ...
            <div class="flex flex-row justify-between" >
                <div class="grid grid-cols-4 gap-y-2 gap-x-5 justify-self-start"><span v-for="tag in ticket?.tags.slice(0,4)" :key="tag.id" class="bg-orange-700 text-white text-center rounded-full px-2 py-1">{{ tag.label }}</span></div>
                <div v-if="ticket?.resolvers.length !=0" class="grid grid-cols-4 gap-y-2 gap-x-5 justify-self-end ">                    
                    <span>Assigné à : </span>
                    <span v-for="user in ticket?.resolvers.slice(0,3)" :key="user.id" class="bg-orange-700 text-white text-center rounded-full px-2 py-1">{{ user.name }}</span>
                </div>
            </div> 
            <div class="flex flex-row justify-between">
                <div class=" flex flex-row space-x-2 items-baseline text-gray-600">
                    ...
                    <div
                        class="flex items-center space-x-3 rounded-full text-orange-500 "
                    >
